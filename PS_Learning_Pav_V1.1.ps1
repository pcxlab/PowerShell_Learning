...
$number = 4
if ($number -gt 3) {
    Write-Host "Number is greater than 3" -ForegroundColor Yellow
}
...
# IF / ELSE
if ($number -eq 10) {
    Write-Host "Number is 10" -ForegroundColor Green
}
else {
    Write-Host "Number is not 10" -ForegroundColor Yellow
}
...
foreach ($abc in $friends) {

    Write-Host "now its looping for $abc" -ForegroundColor Red   # Keeping as-is (even though logically it should be $friend)
    Write-Host "hi $abc How are you doing today?" -ForegroundColor Green
    Write-host "Take Fruit $abc" -ForegroundColor Yellow
}
...
# Calling function
Say-Hello -name "Nick"
Say-Hello -name "Rakesh"
Say-Hello -name "Sandeep"
...
# Overwrite
"New Information" > "C:\TEMP\test.txt"
